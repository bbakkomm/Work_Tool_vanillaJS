...
export default class tradeIn {
  constructor(tradeContainer, tradeData) {
      ...
      this.data = tradeData.result; // json data
  ...
  reDrawHtml(resultData, resetObj, $el, defaultBtnHtml) {
      ...
      reSelect.forEach((reSelectItem, i) => {
          if (reSelectItem === null) {
              ...
              $combobox.find(`.pt_combo__item`).append('<span class="blind">선택됨</span>');
              $combobox.find(`[data-pt-option=${reSelectValue}`).closest('.pt_combo__item').addClass('pt_on');
          }
      });
